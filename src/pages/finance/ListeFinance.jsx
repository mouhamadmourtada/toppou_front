...
const ListeFinance = () => {
    ...
    const goToAddFinance = () => {
        navigate("/app/finance/create");
    };

    const handleEdit = (id) => {
        navigate(`/app/finance/edit/${id}`);
    };

    const handleView = (id) => {
        navigate(`/app/finance/show/${id}`);
    };
            ...
            <div className='my-3 font-semibold'>
                <Breadcrumb>
                    <BreadcrumbList>
                        ...
                        <BreadcrumbSeparator />
                        <BreadcrumbItem>
                            <BreadcrumbPage className="text-primaire font-semibold">Financements</BreadcrumbPage>
                        </BreadcrumbItem>
                    </BreadcrumbList>
                </Breadcrumb>
            </div>
                        ...
                        <table className="table-auto w-full">
                            <thead className='align-left'>
                                <tr className='bg-primaire text-white text-xs font-semi-bold tracking-wide uppercase'>
                                    ...
                                    <th className='px-2 text-left py-3 uppercase'>Date Obtention</th>
                            ...
                            <tbody className='text-tertiaire'>
                                {finances.map((finance, index) => (
                                    <tr key={index} className="whitespace-nowrap">
                                        <td className="px-2 py-3 border-b">{finance.id}</td>
                                        <td className="px-2 py-3 border-b">{finance.description}</td>
                                        <td className="px-2 py-3 border-b">{formatDate(finance.dateObtentionEffective)}</td>
                                        <td className="px-2 py-3 border-b">{finance.objectif}</td>
                                        <td className="px-2 py-3 border-b">{finance.montant}</td>
                                        <td className='border-b flex justify-center items-center h-full text-sm px-3 py-3 border-l'>
                                            <div className="px-4 border-2 border-gray-200 flex justify-center rounded items-center space-x-2 text-sm max-w-20 shadow-xl p-0.5 bg-gray-100">
                                                <div className="text-secondaire cursor-pointer" onClick={() =>  handleView(finance.id)}>
                                                ...
                                                <div className="text-secondaire cursor-pointer" onClick={() =>  handleEdit(finance.id)}>
